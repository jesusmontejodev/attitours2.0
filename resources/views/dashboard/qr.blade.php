...
<!-- 
 * @file qr.blade.php
 * @description Módulo de escaneo de códigos QR de asistencia para validar boarding de pasajeros.
 * @date 2026-06-10
 * @author Antigravity
 -->

@section('title', 'Asistencia por QR - Atti Tours')
...
@section('content')
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-900 pb-6 mb-8">
            <div>
                <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-slate-400 hover:text-cyan-400 transition-colors mb-3 cursor-pointer">
            ...
            <button id="qr-toggle-btn" onclick="toggleQrScanner()"
                class="h-10 px-5 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 text-xs font-black uppercase tracking-wider text-white shadow-lg hover:opacity-90 transition-all cursor-pointer">
                Iniciar Cámara
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-7 flex flex-col gap-6">
                <div class="p-6 rounded-3xl border border-slate-900 bg-slate-950/80 backdrop-blur-md shadow-2xl">
                    <h2 class="text-xs font-black uppercase tracking-widest text-slate-200 border-b border-slate-900 pb-3 mb-5">
                        Lector de Códigos QR
                    </h2>

                    {{-- Estado inicial --}}
                    <div id="qr-idle-state" class="flex flex-col items-center justify-center h-72 rounded-2xl border-2 border-dashed border-slate-800 bg-slate-900/10">
                        <div class="h-16 w-16 rounded-2xl bg-slate-900 flex items-center justify-center mb-4">
                        ...
                        <p class="text-[10px] text-slate-600 mt-1">Haz clic en "Iniciar Cámara" para escanear.</p>
                    </div>

                    {{-- Contenedor de la cámara (html5-qrcode) --}}
                    <div id="qr-reader" class="hidden w-full rounded-2xl overflow-hidden border border-slate-900" style="min-height:300px;"></div>

                    {{-- Input manual como fallback --}}
                    <div id="qr-manual-input" class="hidden mt-6 pt-5 border-t border-slate-900/60">
                        ...
                        <div class="flex gap-2">
                            <input type="text" id="qr-manual-text" placeholder="Pega o escribe el token del QR aquí..."
                                class="flex-1 h-10 rounded-xl border border-slate-800 bg-slate-900 px-3 text-xs text-white placeholder-slate-600 focus:border-cyan-500 focus:outline-none">
                            <button onclick="processQrContent(document.getElementById('qr-manual-text').value)"
                                class="px-5 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-xs font-bold text-cyan-400 hover:bg-cyan-500/20 cursor-pointer transition-all">
                                Validar
                            </button>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-5 flex flex-col gap-6">
                ...
                <div class="p-6 rounded-3xl border border-slate-900 bg-slate-950/80 backdrop-blur-md shadow-2xl">
                    ...
                    {{-- Estado vacío --}}
                    <div id="qr-result-empty" class="flex flex-col items-center justify-center h-48 text-slate-600 text-center">
                        ...
                        <p class="text-[10px] text-slate-500 mt-1 max-w-[200px]">Los datos del ticket aparecerán en este panel una vez se detecte el código.</p>
                    </div>

                    {{-- Loading --}}
                    <div id="qr-result-loading" class="hidden flex flex-col items-center justify-center h-48">
                        <div class="h-10 w-10 rounded-full border-2 border-cyan-500/30 border-t-cyan-500 animate-spin mb-3"></div>
                        <p class="text-xs text-slate-400">Verificando en base de datos...</p>
                    </div>

                    {{-- Resultado confirmado ✅ --}}
                    <div id="qr-result-ok" class="hidden">
                        <div class="flex items-center gap-3 mb-4 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/25">
                            <div class="h-10 w-10 rounded-xl bg-emerald-500/20 flex items-center justify-center flex-shrink-0">
                                <svg class="h-6 w-6 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                                </svg>
                            </div>
                            <div>
                                <p id="qr-ok-title" class="text-sm font-black text-emerald-400">Boarding Permitido</p>
                                <p id="qr-ok-subtitle" class="text-[10px] text-emerald-300/60"></p>
                            </div>
                        </div>
                        <div class="space-y-3" id="qr-client-info"></div>
                        <button onclick="resetScanner()" class="mt-5 w-full h-10 rounded-xl border border-slate-800 bg-slate-900 text-xs font-bold text-slate-300 hover:bg-slate-800 hover:text-white cursor-pointer transition-colors">
                    ...
                    <div id="qr-result-error" class="hidden">
                        <div class="flex items-center gap-3 mb-4 p-4 rounded-2xl bg-rose-500/10 border border-rose-500/25">
                            <div class="h-10 w-10 rounded-xl bg-rose-500/20 flex items-center justify-center flex-shrink-0">
                                <svg class="h-6 w-6 text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                            </div>
                            <div>
                                <p class="text-sm font-black text-rose-400">Boarding Rechazado</p>
                                <p id="qr-error-msg" class="text-[10px] text-rose-300/70"></p>
                            </div>
                        </div>
                        <button onclick="resetScanner()" class="w-full h-10 rounded-xl border border-slate-800 bg-slate-900 text-xs font-bold text-slate-300 hover:bg-slate-800 hover:text-white cursor-pointer transition-colors">
                            Reintentar Escaneo
                        </button>
                    </div>
                </div>
                ...
                <div class="p-6 rounded-3xl border border-slate-900 bg-slate-950/40 shadow-xl overflow-hidden">
                    ...
                    @if($asistenciasHoy->isEmpty())
                        <p class="text-xs text-slate-500 text-center py-6" id="empty-history-label">Ninguna asistencia confirmada hoy</p>
                        <div class="space-y-2 max-h-[250px] overflow-y-auto pr-1" id="history-container"></div>
                    @else
                        <p class="text-xs text-slate-500 text-center py-6 hidden" id="empty-history-label">Ninguna asistencia confirmada hoy</p>
                        <div class="space-y-2 max-h-[250px] overflow-y-auto pr-1" id="history-container">
                            @foreach($asistenciasHoy as $res)
                                <div class="flex items-center justify-between px-4 py-3 rounded-xl border border-emerald-500/10 bg-emerald-500/5 text-xs animate-fade-in">
                                    <div>
    ...
    <script>
        ...
        function startQrScanner() {
            ...
            toggleBtn.classList.remove('from-cyan-500', 'to-indigo-600');
            toggleBtn.classList.add('from-rose-500', 'to-pink-600');
            ...
            html5QrScanner.start(
                ...
                idleState.innerHTML = `
                    <div class="h-16 w-16 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center mb-3 mx-auto">
                        <span class="text-2xl">⚠️</span>
                    </div>
                    <p class="text-xs font-bold text-amber-400 text-center">No se pudo acceder a la cámara</p>
        ...
        function stopQrScanner() {
            ...
            idleState.innerHTML = `
                <div class="h-16 w-16 rounded-2xl bg-slate-900 flex items-center justify-center mb-3">
                    <svg class="h-8 w-8 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
            ...
            toggleBtn.classList.add('from-cyan-500', 'to-indigo-600');
            toggleBtn.classList.remove('from-rose-500', 'to-pink-600');
        ...
        function prependHistoryRow(res) {
            ...
            row.className = 'flex items-center justify-between px-4 py-3 rounded-xl border border-emerald-500/10 bg-emerald-500/5 text-xs animate-fade-in';
            row.innerHTML = `
                <div>
                    <span class="font-bold text-white block">${res.nombre_cliente}</span>
                    <span class="text-slate-500 text-[10px]">${res.ticket_codigo}</span>
        ...
        function showQrResult(data) {
            ...
            if (data.status === 'already_confirmed') {
                ...
                okTitle.className = 'text-sm font-black text-amber-400';
                okSubtitle.textContent = 'Confirmada anteriormente: ' + (data.confirmed_at || '');
            } else {
                okTitle.textContent = '¡Boarding Permitido!';
                okTitle.className = 'text-sm font-black text-emerald-400';
            ...
            if (r) {
                let toursHtml = (r.tours || []).map(t => `
                    <div class="text-[10px] text-slate-400 pl-3 border-l border-slate-700">
                        <span class="font-bold text-slate-300">${t.nombre}</span><br>
                        📅 ${t.fecha} · 🕒 ${t.horario} · 👥 ${t.personas} Pax
                    </div>
                `).join('');

                clientInfo.innerHTML = `
                    <div class="space-y-2.5 p-3 rounded-xl bg-slate-900/60 border border-slate-800">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-xs font-black text-white">${r.nombre_cliente}</p>
                                <p class="text-[10px] text-slate-500">${r.correo}</p>
                                <p class="text-[10px] text-slate-500">${r.telefono || ''}</p>
                            </div>
                            <span class="text-[9px] font-black px-2 py-1 rounded-md bg-slate-800 text-cyan-400 border border-slate-700">${r.ticket_codigo}</span>
                        </div>
