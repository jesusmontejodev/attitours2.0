...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ticket-code { font-size: 16px; font-weight: 900; color: #007a63;
        ...
        .info-block .value { font-size: 13px; font-weight: 700; color: #0f172a; }
        .badge-pagada { display: inline-block; background: #ecfdf5; color: #059669;
        ...
        .tour-meta { font-size: 11px; color: #64748b; line-height: 1.6; }
        .tour-meta span { color: #007a63; font-weight: 600; }
        .tour-price { font-size: 14px; font-weight: 900; color: #0f172a; float: right; margin-top: -20px; }
        ...
        .circle-left  { position: absolute; top: -12px; left:  -28px; width: 22px; height: 22px;
                        background: #f1f5f9; border-radius: 50%; border: 1px solid #f1f5f9; }
        .circle-right { position: absolute; top: -12px; right: -28px; width: 22px; height: 22px;
                        background: #f1f5f9; border-radius: 50%; border: 1px solid #f1f5f9; }

        /* ── QR Section ─────────────────────────── */
        .qr-section { text-align: center; padding: 8px 0 24px; }
        .qr-section h2 { font-size: 14px; font-weight: 800; color: #0f172a; margin-bottom: 4px; }
        ...
        .qr-note { margin-top: 12px; font-size: 10px; color: #94a3b8; }
        .qr-note strong { color: #475569; }

        /* ── Total ──────────────────────────────── */
        .total-row { display: flex; justify-content: space-between; align-items: center;
                     padding: 16px 0 0; }
        .total-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: #94a3b8; font-weight: 700; }
        .total-amount { font-size: 22px; font-weight: 900; color: #007a63; }
    ...
    <div class="ticket">
        ...
        <div class="ticket-header">
            ...
            @if($reserva->monto_pendiente_destino_usd > 0)
                <span class="badge-pagada" style="background: #fff7ed; color: #ea580c; border: 1px solid #fed7aa;">✓ Anticipo Pagado</span>
            @else
        ...
        <div class="ticket-body">
            ...
            {{-- Total --}}
            @if($reserva->monto_pendiente_destino_usd > 0)
                <div class="total-row" style="padding-bottom: 8px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
                    <div class="total-label" style="font-size: 11px;">Monto total del viaje</div>
                    <div class="total-amount" style="font-size: 16px; color: #0f172a;">${{ number_format($reserva->precio_total_usd, 2) }} <span style="font-size:10px;color:#94a3b8;">USD</span></div>
                </div>
                <div class="total-row" style="padding-top: 8px; padding-bottom: 8px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
                    <div class="total-label" style="font-size: 11px; color: #ea580c;">Anticipo pagado online</div>
                    <div class="total-amount" style="font-size: 18px; color: #007a63;">${{ number_format($reserva->monto_pagado_online_usd, 2) }} <span style="font-size:10px;color:#94a3b8;">USD</span></div>
                </div>
                <div class="total-row" style="padding-top: 8px; display: flex; justify-content: space-between; align-items: center;">
                    <div class="total-label" style="font-size: 11px; color: #ed722e;">Restante a pagar en destino</div>
                    <div class="total-amount" style="font-size: 16px; color: #ed722e;">${{ number_format($reserva->monto_pendiente_destino_usd, 2) }} <span style="font-size:10px;color:#94a3b8;">USD</span></div>
                </div>
            @else
                <div class="total-row" style="display: flex; justify-content: space-between; align-items: center; padding: 16px 0 0;">
                    <div class="total-label">Total Pagado</div>
                    <div class="total-amount">${{ number_format($reserva->precio_total_usd, 2) }} <span style="font-size:12px;color:#94a3b8;font-weight:500;">USD</span></div>
                </div>
            @endif

            @if(isset($tempPassword) && $tempPassword)
            <div style="background: #f0fdfa; border: 1px solid #99f6e4; border-radius: 16px; padding: 20px; margin-top: 24px; text-align: left;">
                <h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin-bottom: 8px;">
                    🔑 ¡Hemos creado tu cuenta de usuario!
                </h3>
                <p style="font-size: 11px; color: #64748b; line-height: 1.6; margin-bottom: 12px;">
                ...
                <div style="background: #ffffff; border-radius: 10px; padding: 12px 16px; border: 1px solid #e2e8f0;">
                    <p style="font-size: 11px; color: #64748b; margin: 0 0 6px 0;"><strong>Usuario:</strong> <span style="color: #007a63; font-family: monospace;">{{ $reserva->correo_cliente }}</span></p>
                    <p style="font-size: 11px; color: #64748b; margin: 0;"><strong>Contraseña Temporal:</strong> <span style="color: #ed722e; font-family: monospace; font-weight: bold; letter-spacing: 1px;">{{ $tempPassword }}</span></p>
                </div>
                <p style="font-size: 9px; color: #94a3b8; margin-top: 10px; line-height: 1.4;">
