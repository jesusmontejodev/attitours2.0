<?php
/**
 * @file DemoDataSeeder.php
 * @description Seeder OPCIONAL con tours y reserva de ejemplo para desarrollo/pruebas. No corre por defecto con `php artisan db:seed`; ejecutar manualmente con `php artisan db:seed --class=DemoDataSeeder`. Requiere haber corrido DatabaseSeeder antes (usa el Proveedor y el usuario Cliente base).
 * @date 2026-08-20
 * @author Antigravity
 */
...
{
    /**
     * Seed demo tours and a sample booking.
     */
    public function run(): void
    {
        ...
        if (!$proveedor || !$cliente) {
            $this->command?->error('Ejecuta primero DatabaseSeeder (php artisan db:seed) para crear el Proveedor y el Cliente base.');
            return;
        }

        // Limpiar registros previos de demo si los hubiera para evitar duplicados
        ReservaTour::query()->delete();
        Reserva::query()->delete();
        TourFecha::query()->delete();
        Tour::query()->delete();

        // Crear Tours con soporte multilenguaje (JSON)
        $toursData = [
            [
                'id' => 'tour_cancun',
        ...
        // Crear una Reserva de prueba inicial (coincidente con booking_1)
        ...
        $this->command?->info('Datos de demo creados: 4 tours de ejemplo y 1 reserva de prueba.');
    }
}
